...
@section('stylesheet')
    ...
    <style>
        section#singleBlog_top {
            background: rgba(30, 49, 51, 0.9) none repeat scroll 0 0;
            padding-top: 122px;
        }
        ...
        .courseImg img{
            ...
            width: 100%;
            object-fit: cover;
            border: 2px solid #2687c9;
            border-radius: 5px;
            -moz-border-radius:5px;
            -webkit-border-radius: 5px;
        }

        h3.course-inner-title {
        ...
        .course-at-glance-wrapper-container ul.couser-duration {
            list-style: none;
        }

        .course-at-glance-wrapper-container ul.couser-duration li{
        ...
        .course-at-glance-wrapper-container ul.couser-duration li i{
            margin-right: 5px;
        }
        ...
        .course_sidebar .sideBlock h4{
            position: relative;
            padding-bottom: 10px;
            margin-bottom: 10px;
        }

        .course_sidebar .sideBlock h4::after{
            content: "";
            width: 35%;
            height: 3px;
            background: #4f81bdcf;
        ...
        #blog_single.certificationCourse_details .container.course_shortInfo,
        section#singleBlog_top section.course-avarage-ratting .container{
            padding: 0px;
            background: none;
        }
...
@section('content')
    ...
    <section id="blog_single" class="certificationCourse_details">
        ...
        <section id="singleBlog_top">
            <div class="container course_shortInfo">
                <div class="row">
                    <div class="col-md-3">
                        <div class="courseImg">
                            @if(!empty($pageDetails->image))
                                <!-- <div class="row">
                        <div class="col-md-12"> -->
                                <img src="{{ asset('/'.$pageDetails->image) }}" height="" width="" alt="Course Images">
                                <!-- </div>
                            </div> -->
                            @endif
                        </div>
                    ...
                    <div class="col-md-9">
                        @if(!empty($pageDetails->tittle))
                            <h3 class="course-inner-title">{!! $pageDetails->tittle !!}</h3>
                        @endif
                        @if(!empty($pageDetails->shortDescription))
                        <p class="course-inner-desc">{!! $pageDetails->shortDescription !!}</p>
                        @endif
                    </div>
                </div>

            </div>
            <section class="course-avarage-ratting">
                <div class="container">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="course-at-glance-wrapper-container">
                                ...
                                <ul class="couser-duration">
                                    @if(!empty($pageDetails->courseDate))
                                    <li> <span><i class="fa fa-calendar-check-o" aria-hidden="true"></i> Date :</span>{!! $pageDetails->courseDate !!}</li>
                                    @endif
                                    @if(!empty($pageDetails->classes))
                                    <li> <span><i class="fa fa-list" aria-hidden="true"></i>No. of Classes/ Sessions
                                            :</span> {!! $pageDetails->classes !!}
                                    </li>
                                    @endif
                                    @if(!empty($pageDetails->duration))
                                    <li> <span><i class="fa fa-clock-o" aria-hidden="true"></i>Total Hours :</span> {!! $pageDetails->duration !!}</li>
                                    @endif
                                    @if(!empty($pageDetails->duration))
                                    <li> <span><i class="fa fa-calendar-check-o" aria-hidden="true"></i>Last Date of
                                            Registration :</span> {!! $pageDetails->lastDate !!} </li>
                                    @endif
                                    @if(!empty($pageDetails->schedule))
                                    <li style="display: block"> <span><i class="fa fa-clock-o"
                                                                         aria-hidden="true"></i>Class
                                            Schedule :</span> <br>
                                        <ul class="courser-schdule-wrape">
                                            {!! $pageDetails->schedule !!}
                                        </ul>
                                    </li>
                                    @endif

                                    @if(!empty($pageDetails->venue))
                                        <li>
                                            <span><i class="fa fa-map-marker" aria-hidden="true"></i>venue :</span> {!! $pageDetails->venue !!}
                                        </li>
                                    @endif
                        ...
                        <div class="col-md-6">
                            <div class="course-price">
                                <h3 class="page-inner-title">
                                    <span class="vat-price">
                                        {!! $pageDetails->amount !!}
                                    </span>
                                </h3>
                                <span style="font-size: 16px">(including VAT &amp; TAX)</span>
{{--                                <p>Early Bird Admission is going on till 17th October, 2023 at BDT 22,500 &amp; One-time--}}
{{--                                    full payment: BDT 21,000</p>--}}
                                <div class="enroll-btn-container">
                                    <a href="{{url('pages/program-apply/'.$pageDetails->page_id.'/'.$pageDetails->course_id)}}" class="enroll_now modalToggle btn-hover-effect">Enroll
                                        Now</a>
            ...
            <div class="row">
                        ...
                        <div class="controls">
                            <ul class="nav nav-tabs">
                                @if($pageDetails->aboutShow == 'Y')
                                    <li class="active"><a  class="active" data-toggle="tab" href="#about">About</a>
                                @endif
                ...
                <div class="col-lg-3">
                    <div class="course_sidebar">
                        <div class="sideBlock">
                            <h4>Tentative Class Start</h4>
                            <p>19th January, 2024</p>
                        </div>
                        <div class="sideBlock">
                            <h4>Available Seat</h4>
                            <p>20/40</p>
                            <button type="submit" class="btn btn-primary">Applay Now</button>
                        </div>
                        <div class="sideBlock">
                            <h4>Who Can Join</h4>
                            <p>People who have basic Computer knowledge.</p>
                        </div>
                    </div>
                </div>
